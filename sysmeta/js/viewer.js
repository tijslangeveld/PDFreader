...
// A comparison bundles one PDF per document, under the letter its citations
// carry: source_A.enc, source_B.enc… Decrypted separately and only when a reader
// actually opens one, so three big reports cost nothing until they are asked for.
const PDFDOCS = new Map();      // entry name -> PDF.js document
const pdfLoadingBy = new Map(); // entry name -> in-flight promise
let SOURCES = [];               // [{key, title, pageCount, has}] for a comparison
...
// A chart carries its data — and the sources it quotes, "[A p.30]" — in an
// attribute. Rewriting that into markup would break the attribute it sits in,
// so it is held aside while the text is converted, then put back.
function withChartDataHeld(html, fn) {
  const held = [];
  const masked = String(html || '').replace(/ data-chart="[^"]*"/g, (m) => {
    held.push(m);
    return ` data-chart-held="${held.length - 1}"`;
  });
  return fn(masked).replace(/ data-chart-held="(\d+)"/g, (m, i) => held[Number(i)] || '');
}

// A document's own colour for its citations — text steps, applied on render so a
// bundle made from an older analysis gets them as well.
const CITE_INK_FALLBACK = ['#1c5cab', '#a8431c', '#067a54', '#8a5a00', '#a8336b', '#006300'];
function citeInk(key) {
  const i = Math.max(0, (String(key || 'A').toUpperCase().charCodeAt(0) - 65) % CITE_INK_FALLBACK.length);
  return `var(--cite-${i + 1}, ${CITE_INK_FALLBACK[i]})`;
}

function citeToSup(html) {
  return withChartDataHeld(html, (html) => {
  ...
  html = html.replace(new RegExp('<sup(?![^>]*pdf-cite)([^>]*)>(' + MULTI + ')<\/sup>', 'g'),
    (m, had, inner, key, pages) =>
      '<sup' + (had || '').replace(/\s*data-doc="[^"]*"/g, '').replace(/\s*style="[^"]*"/g, '') +
      attrs(pages) + ' data-doc="' + key + '"' +
      ' style="color:' + citeInk(key) + ';font-weight:600">' + inner + '</sup>');
  html = html.replace(new RegExp('(<sup[^>]*>)?(' + MULTI + ')(<\/sup>)?', 'g'),
    (m, pre, inner, key, pages) => (pre ? m : '<sup' + attrs(pages) + ' data-doc="' + key + '"' +
      ' style="color:' + citeInk(key) + ';font-weight:600">' + inner + '</sup>'));
...
function render(data) {
  ...
  if (kicker) kicker.textContent = isTk ? 'Tweede Kamer meta-analyse'
    : isMulti ? 'Vergelijkende analyse · ' + SOURCES.length + ' documenten'
    : 'PDF summary';
  ...
  // A comparison carries one entry per document; "has a PDF" means at least one
  // of them made it into the bundle.
  const entryFor = (key) => 'source_' + key;
  const hasEntry = (name) => !!(MANIFEST.entries && MANIFEST.entries[name]);
  const hasPdf = isMulti
    ? SOURCES.some((sv) => hasEntry(entryFor(sv.key)))
    : hasEntry('source');
  ...
  const parts = isTk
    ? wrapSections(wrapTables(data.html || ''), IS_INTRO)
    : wrapSections(wrapTables(citeToSup(mdToHtml(data.html || ''))));
  // What A, B and C stand for — and the way into each original, since a
  // comparison has no single "the source PDF" to offer.
  const roster = isMulti ? '<div class="doc-key-block">' +
    '<div class="doc-key-head">Bronnen en kleurcodering</div>' +
  ...
  bodyEl.innerHTML =
    ...
    (parts.sections || hasPdf ? sectionsBar(hasPdf && !isMulti) : '') +
  ...
  if (isMulti) bodyEl.addEventListener('click', (e) => {
    const it = e.target.closest && e.target.closest('.doc-roster-item');
    if (!it || it.classList.contains('is-inert')) return;
    const sv = SOURCES.filter((x) => x.key === it.dataset.key)[0] || {};
    openPagePopup(1, [], entryFor(it.dataset.key), it.dataset.key + ' · ' + (sv.title || ''));
  });
  ...
  bodyEl.addEventListener('click', (e) => {
    ...
    const page = parseInt(cite.getAttribute('data-page'), 10) || 1;
    const key = cite.getAttribute('data-doc');
    if (key) {
      // Not in this bundle (exported with --no-pdf, or never archived): the
      // citation stays readable, it just has nothing to open.
      if (!hasEntry(entryFor(key))) return;
      const sv = SOURCES.filter((x) => x.key === key)[0] || {};
      openPagePopup(page, pages, entryFor(key), key + ' · ' + (sv.title || ''));
      return;
    }
    openPagePopup(page, pages);
  });

  try { wireUnitToggle(data); } catch (e) { console.warn('[viewer] unit toggle:', e); }
  ...
  $('doc-foot').textContent = isTk
    ...
    : isMulti
      ? (hasPdf
        ? 'Elke verwijzing draagt de letter van het document waar hij vandaan komt — klik erop om die pagina in het originele PDF-document te openen.'
        : 'De originele PDF-documenten zijn niet meegeleverd; paginaverwijzingen zijn niet aanklikbaar.')
    : hasPdf
...
// captions. 1 TWh = 3,6 PJ.
//
// Each number is wrapped once, on first render, in a span that remembers what
...
function parseNlNumber(str) {
  ...
  const cleaned = String(str).replace(/\.(?=\d{3}\b)/g, "").replace(",", ".");
  const n = parseFloat(cleaned);
  return Number.isFinite(n) ? n : null;
}
function fmtNlNumber(n) {
  const abs = Math.abs(n);
  const d = abs >= 100 ? 0 : abs >= 10 ? 1 : abs >= 1 ? 2 : 3;
  return n.toLocaleString("nl-NL", { maximumFractionDigits: d });
}

function markUnitValues(root) {
  const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
    acceptNode(node) {
      // A chart's own labels are not text to be converted: the bundle carries a
...
function applyUnit(unit, toggle) {
  const to = toggle.to, from = toggle.from, factor = toggle.factor || 1;
  document.documentElement.dataset.unit = unit;
  // The chart drawn in this unit; the other one steps aside.
  document.querySelectorAll('.unit-view').forEach((v) => {
    v.hidden = v.dataset.unit !== unit;
  });
...
function loadPdf(entry) {
  ...
  if (PDFDOCS.has(name)) return Promise.resolve(PDFDOCS.get(name));
  if (pdfLoadingBy.has(name)) return pdfLoadingBy.get(name);
  const p = openEntry(name)
    .then((buf) => pdfjsLib.getDocument({ data: new Uint8Array(buf) }).promise)
    .then((doc) => {
      PDFDOCS.set(name, doc);
      pdfLoadingBy.delete(name);
      return doc;
    })
    .catch((err) => { pdfLoadingBy.delete(name); throw err; });
  pdfLoadingBy.set(name, p);
  return p;
}
...
function openPagePopup(page, pages, entry, label) {
  const ov = document.createElement('div');
  ov.className = 'pdfpage-overlay';
  ov.innerHTML =
    '<div class="pdfpage-popup" role="dialog" aria-label="Bronpagina">' +
      '<div class="pdfpage-head">' +
        '<div class="pdfpage-title">Bronpagina <span class="pdfpage-num"></span>' +
          (label ? '<span class="pdfpage-doc">' + escHtml(label) + '</span>' : '') +
        '</div>' +
  ...
  const refList = (pages && pages.length ? pages : [page])
    ...
    .sort((a, b) => a - b);
  const entryName = entry || 'source';
  // The bounds and the page count belong to the document this popup opened, not
  // to whichever one happened to be decrypted first.
  const thisDoc = () => PDFDOCS.get(entryName) || null;
  let cur = page;
  let mode = 'page';
  ...
  function step(dir) {
    const next = cur + dir;
    const doc = thisDoc();
    if (next < 1 || (doc && next > doc.numPages)) return;
    cur = next;
    draw();
  }
  ...
  openLink.addEventListener('click', async (e) => {
    ...
    e.preventDefault();
    try {
      const doc = await loadPdf(entryName);
      const bytes = await doc.getData();
  ...
  function syncChrome() {
    const doc = thisDoc();
    numEl.textContent = String(cur) + (doc ? ' / ' + doc.numPages : '');
    if (refList.length > 1) {
    ...
    navs[1].disabled = !!(doc && cur >= doc.numPages);
  }

  async function draw() {
    const mine = ++token;
    syncChrome();
    if (!thisDoc()) body.innerHTML = '<div class="pdfpage-loading">Document ontsleutelen…</div>';
    try {
      const doc = await loadPdf(entryName);
